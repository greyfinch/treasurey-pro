    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500">Security and activity trail for the organisation</p>
            </div>
            <div class="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100 text-xs font-semibold">
        ...
        <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div v-if="logs.length === 0" class="p-20 text-center">
                ...
                <p class="text-xs text-gray-400 mt-1">Audit logs are generated when you perform sensitive actions like creating investments or recordings withdrawals.</p>
            </div>
                        ...
                        <tr v-for="log in logs" :key="log.id" class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-xs text-gray-400 capitalize">{{ log.userRole.toLowerCase().replace(/_/g, ' ') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">
                                ...
                                <div v-else-if="log.action === 'withdrawal:create'">
                                    Amount: <span class="font-semibold text-red-600">{{ formatCurrency(log.details.amount) }}</span>
                                </div>
                                <div v-else-if="log.action === 'rollover:create'">
                                    Injection: <span class="font-semibold text-green-600">{{ formatCurrency(log.details.amount) }}</span>
                                </div>
