...
<template>
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Subsidiary Management</h1>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="org in organisations" :key="org.id" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div>
                    ...
                    <p class="text-xs text-gray-500 font-mono mb-4">ID: {{ org.id }}</p>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 border-t border-gray-50 pt-4 mt-2">
                    ...
                    >
                        ...
                        <div v-else class="w-5 h-5 border-2 border-red-600 border-t-transparent animate-spin rounded-full"></div>
                    </button>
                </div>
            </div>
        </div>
